...
Let $P(x,y,t)$ be the probability density function of the process, where $A$ is labelled as species 1 (taking on the values corresponding to $x\in\mathbb{R}_+$) and $B$ as species 2 (taking on the values corresponding to $y\in\mathbb{R}_+$). The propensity functions for the reaction process are:
...
The corresponding molecules number adjustments in the chemical Fokker-Planck notation are thus given by:

$$
\begin{aligned}
&v_{1,1} = -2,\, v_{1,2} = 0, \quad v_{2,1} = -1, \, v_{2,2} = -1, \\
&v_{3,1} = +1, \, v_{3,2} = 0, \quad v_{4,1} = 0, \, v_{4,2} = +1.
\end{aligned}
$$

Hence the chemical Fokker-Planck equations for the process are given by:

$$
\begin{aligned}
\frac{\partial}{\partial t}P(x,y,t) &= 
-\frac{\partial}{\partial x}\left[\left( \frac{-2k_1 x(x-1)}{\nu} - \frac{k_2xy}{\nu} + \nu k_3 \right)P(x,y,t)\right] \\
&-\frac{\partial}{\partial y}\left[\left( - \frac{k_2xy}{\nu} + \nu k_4 \right)P(x,y,t)\right] \\
&+\frac{1}{2}\frac{\partial^2}{\partial x^2} \left[ \left( \frac{4k_1 x(x-1)}{\nu} + \frac{k_2 x y}{\nu} + \nu k_3 \right)P(x,y,t) \right] \\
&+\frac{\partial^2}{\partial x \partial y} \left[ \left( \frac{k_2 x y}{\nu} \right)P(x,y,t) \right]\\
&+\frac{1}{2}\frac{\partial^2}{\partial y^2} \left[ \left( \frac{k_2 x y}{\nu} + \nu k_4 \right)P(x,y,t) \right].
\end{aligned}
$$
...
- Derive hitting time formulae and use them to compute hitting times for SDEs.
