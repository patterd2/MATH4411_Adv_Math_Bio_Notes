...
Virtually all modern programming languages have algorithms for generating (pseudo-) random numbers and we can use such routines to generate the uniformly distributed random numbers called for in step 1 to execute the **naive SSA**. The output of some simulations of the degradation process using this algorithm are shown in Figure 1. The left panel shows two sample paths compared to what we called the **deterministic mean**. The motivation for this comparison is that a principle called **the law of mass action** can be invoked to yield a simple deterministic model of the reaction process.
...
$$
The deterministic model of the mean behaviour is strictly decreasing in time and thus does not capture the qualitative behaviour of individual trajectories very well. However, the right panel of Figure 1 shows that it captures the behaviour of the average over a large number of simulations quite well; the stochastic mean at time $t$ is calculated as the average value at that time over $20$ simulations of the process.

![Left: Some paths of the degradation process compared to the corresponding deterministic model of the same process. Right: Stochastic mean compared to the mean predicted by the deterministic model. Parameters: $n_0 = 20$, $k = 0.1$, $\Delta t = 0.005$.](images/Figure1_degradation_example.png){width=95% fig-align="center"}
...
Note that these calculations were facilitated by the fact that, given positive rate constants, volume and a non-zero number of molecules, we have
...
The chemical master equations for complex higher-order reaction processes are typically very hard to solve analytically. In this section we will use an example of a dimerisation process to introduce the probability generating function approach to solving the chemical master equations.
...
To simulate the process, we need to compute the overall propensity function for the process

$$
\alpha_0(t) = \alpha_1(t) + \alpha_2(t) + \alpha_3(t) + \alpha_4(t),
$$

to compute the next reaction time. If $\tau$ is the next reaction time, then the step to update the number of $A$ and $B$ molecules according to the Gillespie algorithm will be:
...
![Sample paths and deterministic solutions. Parameters: $k_1 = 0.001$, $k_2 = 0.01$, $k_3 = 1.2$, $k_4 = 1$.](images/gillespie.png){#fig-gillespie width=100% fig-align="center"}

We can gain more information about the dynamics of the process by running more and longer simulations. This allows us to estimate the stationary distribution which characterizes the asymptotic behaviour of the process. Figure @fig-gillespie2 shows the estimated stationary distribution in the left-hand panel, i.e. the joint PMF of $A(t)$ and $B(t)$ as $t \to \infty$. The right-hand panel of @fig-gillespie2 shows the marginal stationary distribution of $A(t)$, which can be obtained from the joint stationary distribution via the formula
